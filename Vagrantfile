...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define :dev do |dev_config|
    dev_config.vm.network :private_network, ip: '33.33.33.33'
    dev_config.vm.hostname = 'livingoz.com'

    dev_config.vm.provision :shell, :path => 'install.sh'



    VAGRANT_JSON = JSON.parse(Pathname(__FILE__).dirname.join('chef/nodes', 'all.json').read)
